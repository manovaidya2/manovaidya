...
const sectionHeadingMap = new Map([
  ...
  ["Why Families Choose Manovaidya", "why-families-trust-manovaidya"],
  ["Meet Dr. Ankush Garg", "meet-dr-ankush-garg"],
...
const pageLinks = [
  ...
  { label: "Common Concerns", id: "common-behavioural-concerns-in-children" },
  { label: "What Behaviour Reveals", id: "understanding-what-behaviour-really-reveals" },
  { label: "Assessment", id: "behavioural-assessment-process" },
  { label: "Child Behaviour Support", id: "child-behaviour-support-in-india" },
  { label: "Neuro-Ayurveda", id: "neuro-ayurveda-development-approach" },
  { label: "Support", id: "supporting-children-with-behavioural-challenges" },
  { label: "Parent Guidance", id: "parent-guidance-and-family-support" },
  { label: "How We Work", id: "our-holistic-approach-at-manovaidya" },
  { label: "Why Manovaidya", id: "why-families-trust-manovaidya" },
  { label: "Dr. Ankush Garg", id: "meet-dr-ankush-garg" },
  { label: "FAQs", id: "frequently-asked-questions" },
  { label: "Consultation", id: "book-consultation" },
];
...
const socialLinks = [
  { label: "Facebook", href: "https://www.facebook.com/manovaidya/", icon: "f" },
  { label: "Instagram", href: "https://www.instagram.com/manovaidya/", icon: "ig" },
  { label: "LinkedIn", href: "https://www.linkedin.com/company/manovaidya/", icon: "in" },
  { label: "YouTube", href: "https://www.youtube.com/@manovaidya", icon: "yt" },
];

const articleLinkRules = [
  { label: "Child Behaviour Assessment", href: "#behavioural-assessment-process", pattern: /Child Behaviour Assessment/i },
  { label: "Behavioural Assessment", href: "#behavioural-assessment-process", pattern: /Behavioural Assessment/i },
  { label: "Behavioural Concerns in Children", href: "#what-are-behavioural-concerns-in-children", pattern: /Behavioural Concerns in Children/i },
  { label: "Child Behaviour Support", href: "#child-behaviour-support-in-india", pattern: /Child Behaviour Support/i },
  { label: "Parent Guidance", href: "#parent-guidance-and-family-support", pattern: /Parent Guidance/i },
  { label: "Family Support", href: "#parent-guidance-and-family-support", pattern: /Family Support/i },
  { label: "Child Development Assessment", href: "/child-development-care-india", pattern: /Child Development Assessment/i },
  ...
  { label: "Sensory Processing Issues", href: "#neuro-ayurveda-development-approach", pattern: /Sensory Processing Issues/i },
...
function getFaqs() {
  ...
  lines.forEach((line) => {
    if (line.endsWith("?")) {
      if (currentQuestion) faqs.push({ question: currentQuestion, answer: answerLines.join(" ") });
...
function LinkifiedText({ text }) {
  ...
  return parts.map((part, index) =>
    urlPattern.test(part) ? (
      <a key={`${part}-${index}`} href={part} target="_blank" rel="noreferrer" className="break-all text-[#7835A4] underline">
        {part}
      </a>
    ) : (
...
function ArticleLinkedText({ text, onCitationClick, allowedRuleKeys }) {
  ...
  while (cursor < source.length && safety < 160) {
    safety += 1;
    const remaining = source.slice(cursor);
    const candidates = [];
    ...
    if (urlMatch) {
      candidates.push({ index: urlMatch.index, text: urlMatch[0], type: "url" });
    }
    ...
    if (next.type === "citation") {
      ...
      parts.push(
        <a key={`${next.text}-${absoluteIndex}`} href={next.text} target="_blank" rel="noreferrer" className="break-all text-[#7835A4] underline">
          {next.text}
        </a>
      );
    } else {
    ...
    <section className="mt-8 overflow-hidden rounded-[28px] bg-[#fbf5ef] shadow-[0_14px_30px_rgba(58,31,90,0.06)]">
      <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="bg-[#f3eadb] p-6 sm:p-8">
          <h3 className="text-[16px] font-black leading-tight text-[#111827]">{behaviourSubtitle}</h3>
          <p className="mt-3 text-[13px] font-semibold leading-6 text-[#111827]">
            Child behaviour assessment, parent guidance and developmental support at Manovaidya.
          </p>
          <a href="#book-consultation" className="mt-5 inline-flex h-[48px] items-center justify-center rounded-full bg-[#0b8f98] px-6 text-[13px] font-black uppercase text-white transition hover:bg-[#087982]">
...
function ArticleLine({ line, index, onCitationClick, allowedRuleKeys, faqAnswerRuleKeys }) {
  const absoluteIndex = (bodyStartIndex === -1 ? 0 : bodyStartIndex) + index;
          ...
          {faqs.map((faq, faqIndex) => (
            <details key={faq.question} open={faqIndex === 0} className="border-b border-[#eadff1] last:border-b-0">
              ...
              <p className="px-4 pb-4 text-[12.5px] font-semibold leading-6 text-[#5f5367]">
                <ArticleLinkedText text={faq.answer} onCitationClick={onCitationClick} allowedRuleKeys={faqAnswerRuleKeys?.get(faq.question)} />
              </p>
            </details>
          ))}
        </div>
  ...
  if (trimmed === "Book a Consultation") return null;

  const sectionId = sectionHeadingMap.get(trimmed);
  const isQuestion = trimmed.endsWith("?");
  const isSubheading = !sectionId && !isQuestion && trimmed.length < 90 && !trimmed.startsWith("[") && !trimmed.startsWith("http") && !/[.!]$/.test(trimmed);

  if (sectionId || isQuestion) {
    const headingId = sectionId || slugifyHeading(trimmed);
    return (
      <h2 key={`${trimmed}-${index}`} id={headingId} className="scroll-mt-28 pt-5 text-[20px] font-black leading-tight text-[#17111f]" style={{ whiteSpace: "pre-wrap" }}>
  ...
  return (
    <p key={`${trimmed}-${index}`} className="text-[14px] font-semibold leading-7 text-[#51465a]" style={{ whiteSpace: "pre-wrap" }}>
      <ArticleLinkedText text={text} onCitationClick={onCitationClick} allowedRuleKeys={allowedRuleKeys} />
    </p>
  );
}
...
function FloatingShareBar() {
  ...
  const encodedTitle = encodeURIComponent(behaviourTitle);
  const shareItems = [
    ...
    { label: "LinkedIn", href: `https://www.linkedin.com/shareArticle?mini=true&url=${encodedUrl}&title=${encodedTitle}`, className: "bg-[#0A66C2]", icon: "in" },
  ...
  return (
    <div className="fixed left-0 top-1/2 z-40 hidden -translate-y-1/2 flex-col shadow-[0_8px_24px_rgba(15,23,42,0.16)] sm:flex">
      {shareItems.map((item) => (
        <a key={item.label} href={item.href} target={item.label === "Email" ? undefined : "_blank"} rel={item.label === "Email" ? undefined : "noreferrer"} aria-label={`Share on ${item.label}`} className={`flex h-[34px] w-[34px] items-center justify-center border-b border-white/35 text-white transition hover:w-10 ${item.className}`}>
          {typeof item.icon === "string" ? <span className="text-[11px] font-black">{item.icon}</span> : item.icon}
        </a>
      ))}
    </div>
  );
...
function BehaviouralConcernsChildrenIndiaPage() {
  ...
  const encodedPageTitle = encodeURIComponent(behaviourTitle);
    ...
    <main className="min-h-screen bg-white text-[#1f1726]" style={{ font: '400 1.125rem/1.5 "Martel", serif' }}>
      ...
      <section className="mx-auto grid gap-7 px-4 py-8 sm:px-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:px-10">
        <article className="min-w-0">
          <div className="relative overflow-hidden rounded-lg pb-6 pt-2 lg:grid lg:min-h-[360px] lg:grid-cols-[minmax(0,1fr)_minmax(330px,0.82fr)] lg:items-center lg:gap-8 lg:pb-8 lg:pt-3">
            <div className="relative z-10">
              <nav className="mb-3 flex flex-wrap items-center gap-2 text-[11px] font-bold text-[#7d7085]" aria-label="Article breadcrumb"><a href="/" className="transition hover:text-[#7835A4]">Home</a><span>/</span><a href="/child-health-care" className="transition hover:text-[#7835A4]">Child Health Care</a><span>/</span><span className="text-[#4b345d]">{behaviourTitle}</span></nav>
              <p className="text-[13px] font-black uppercase tracking-[0.08em] text-[#7835A4]">{behaviourTitle}</p>
              <h1 className="mt-2 max-w-[650px] text-[27px] font-black leading-[1.12] text-[#292064] sm:text-[34px] lg:text-[34px]">{behaviourTitle}</h1>
              <p className="mt-4 max-w-[620px] text-[14px] font-bold leading-7 text-[#51465a] sm:text-[15px]">{behaviourSubtitle}</p>
              <div className="mt-6 flex flex-wrap items-center gap-3"><a href="#book-consultation" className="inline-flex min-h-11 items-center justify-center rounded-md bg-[#7835A4] px-5 text-[12px] font-black text-white shadow-[0_10px_20px_rgba(120,53,164,0.22)] transition hover:bg-[#5f2785]">Book Behaviour Assessment</a><a href="tel:+917823838638" className="inline-flex min-h-11 items-center justify-center rounded-md border border-[#7835A4] bg-white px-5 text-[12px] font-black text-[#7835A4] transition hover:bg-[#f5ebfb]">Talk to Our Team</a></div>
              <div className="mt-6 flex flex-wrap items-center gap-5"><p className="text-[12px] font-bold text-[#75677d]">Behaviour Assessment - Parent Guidance</p><div className="flex items-center gap-3 text-[12px] font-black text-[#75677d]"><span>Share:</span>{articleShareLinks.map(({ label, href, title }) => (<a key={label} href={href} target="_blank" rel="noreferrer" className="flex h-9 w-9 items-center justify-center rounded-full bg-[#f2eaf7] text-[#7835A4] transition hover:bg-[#7835A4] hover:text-white" aria-label={title}>{label === "wa" ? <Share2 className="h-4 w-4" /> : <span className="text-[11px] font-black">{label}</span>}</a>))}</div></div>
            </div>
            <figure className="relative z-10 mx-auto mt-8 w-full max-w-[520px] lg:mt-0 lg:max-w-none"><div className="-translate-y-[50px] overflow-hidden rounded-[48%_52%_46%_54%/58%_42%_58%_42%] border-[6px] border-white bg-[#f3eaf9] shadow-[0_18px_35px_rgba(58,31,90,0.12)] lg:-translate-y-[58px]"><img src={childHeroImage} alt="Child behaviour assessment parent guidance and developmental support at Manovaidya" loading="eager" fetchPriority="high" decoding="async" sizes="(min-width: 1024px) 42vw, 100vw" className="h-[248px] w-full object-cover object-right sm:h-[315px] lg:h-[355px]" /></div></figure>
          </div>
        ...
        <div className="sticky top-24 max-h-[calc(100vh-6rem)] space-y-5 overflow-y-auto pb-5">
          <SidebarCard className="border-[#d8c6e5] bg-[#fcf9ff]"><a href="/about/doctor" className="flex items-start gap-3"><img src={doctorImage} alt="Dr Ankush Garg Manovaidya behavioural concerns child development and mental health clinician" className="h-12 w-12 shrink-0 rounded-full object-cover" loading="lazy" decoding="async" /><div><p className="text-[13px] font-black leading-5 text-[#21142d]">Authorised & Clinically Reviewed by Dr Ankush Garg</p><p className="mt-2 text-[12px] font-bold leading-5 text-[#75677d]">Founder, Manovaidya | Ayurvedacharya | Creator of the Neuro-Ayurveda Development System</p><p className="mt-2 text-[12px] font-bold leading-5 text-[#75677d]">Clinical Focus: Autism, ADHD, Child Development and Mental Health</p><p className="mt-2 text-[12px] font-bold leading-5 text-[#75677d]">Last Updated: [20-07-2026]</p></div></a><div className="mt-4 flex items-center gap-3 rounded-lg bg-[#5d3b90] p-3 text-white"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/15"><Brain className="h-5 w-5" /></span><p className="text-[12px] font-black leading-5">Neuro-Ayurveda Development System</p></div></SidebarCard>
          <SidebarCard><p className="text-[15px] font-black text-[#21142d]">Follow Manovaidya</p><div className="mt-4 flex items-center gap-3">{socialLinks.map(({ label, href, icon }) => (<a key={label} href={href} target="_blank" rel="noreferrer" className="flex h-9 w-9 items-center justify-center rounded-full bg-[#7835A4] text-[10px] font-black uppercase text-white shadow-[0_10px_22px_rgba(120,53,164,0.22)] transition hover:bg-[#632b86]" aria-label={label}>{icon}</a>))}</div></SidebarCard>
          <SidebarCard><p className="text-[15px] font-black text-[#21142d]">Search</p><label className="mt-4 flex h-11 items-center rounded-lg border border-[#eadff1] bg-[#fbf9fd] px-3"><input type="search" placeholder="Search topics..." className="min-w-0 flex-1 bg-transparent text-[12px] font-bold text-[#21142d] outline-none placeholder:text-[#9b8ea5]" /><Search className="h-4 w-4 text-[#7835A4]" /></label></SidebarCard>
          <SidebarCard><p className="text-[15px] font-black text-[#21142d]">On this page</p><nav className="mt-5 space-y-3.5">{pageLinks.map(({ label, id }) => (<a key={id} href={`#${id}`} onClick={() => setActiveSection(id)} className={`flex items-start gap-2 rounded-md px-2 py-1.5 text-[12px] font-bold leading-5 transition ${activeSection === id ? "bg-[#f0e7f6] text-[#7835A4]" : "text-[#51465a] hover:bg-[#fbf7ff] hover:text-[#7835A4]"}`}><span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${activeSection === id ? "bg-[#7835A4]" : "bg-[#c5adcf]"}`} />{label}</a>))}</nav></SidebarCard>
          <SidebarCard><p className="text-[15px] font-black text-[#21142d]">Categories</p><div className="mt-4 space-y-3">{categories.map(({ label, count, Icon, href }) => (<a key={label} href={href} className="flex items-center justify-between text-[12px] font-bold text-[#51465a] transition hover:text-[#7835A4]"><span className="flex items-center gap-2"><Icon className="h-4 w-4 text-[#7835A4]" />{label}</span><span>{count}</span></a>))}</div></SidebarCard>
          <RelatedPagesCard />
          <SidebarCard className="border-[#d8c6e5] bg-[#fcf9ff]"><CalendarCheck className="h-9 w-9 text-[#7835A4]" /><p className="mt-4 text-[17px] font-black text-[#21142d]">Book a Consultation</p><p className="mt-3 text-[12px] font-bold leading-5 text-[#5f5367]">Get a personalized behaviour support plan for your child.</p><a href="tel:+917823838638" className="mt-4 inline-flex h-10 items-center justify-center rounded-md bg-[#7835A4] px-5 text-[12px] font-black text-white transition hover:bg-[#632b86]">Book Now</a><p className="mt-4 text-[12px] font-bold text-[#51465a]">Noida, Uttar Pradesh, India</p></SidebarCard>
        </div>
      </section>

      <section className="border-y border-[#eadff1] bg-white"><div className="mx-auto grid max-w-[1440px] gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[1fr_460px] lg:items-center lg:px-8"><div className="flex items-center gap-4"><span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-[#f0e7f6] text-[#7835A4]"><Mail className="h-8 w-8" /></span><div><h2 className="text-[18px] font-black text-[#21142d]">Stay Updated with Insights That Matter</h2><p className="mt-1 text-[13px] font-bold text-[#5f5367]">Subscribe to expert articles, behaviour guidance and Manovaidya updates.</p></div></div><form className="flex gap-3"><input type="email" placeholder="Enter your email address" className="min-w-0 flex-1 rounded-md border border-[#eadff1] px-4 text-[12px] font-bold outline-none focus:border-[#7835A4]" /><button className="h-11 rounded-md bg-[#7835A4] px-6 text-[12px] font-black text-white transition hover:bg-[#632b86]">Subscribe</button></form></div></section>
    </main>
  );
}
