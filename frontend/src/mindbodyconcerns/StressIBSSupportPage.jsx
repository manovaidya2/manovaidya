...
const pageLinks = [
  ...
  { label: "What is IBS?", id: "what-is-irritable-bowel-syndrome-ibs" },
  { label: "Stress Connection", id: "how-are-stress-and-ibs-related" },
  { label: "Gut-Brain Axis", id: "the-gut-brain-connection-and-ibs" },
  { label: "Symptoms", id: "common-symptoms-of-ibs" },
  { label: "Warning Signs", id: "common-signs-to-get-checked" },
  { label: "Assessment Process", id: "our-stress-and-ibs-assessment-process" },
  { label: "Neuro-Ayurveda", id: "the-neuro-ayurveda-development-system" },
  { label: "Support Approach", id: "how-we-support-individuals-with-stress-and-ibs" },
...
const faqEnd = rawSourceContent.indexOf("\nDevelop a Greater Understanding of IBS?", faqStart);
...
const articleSections = articleContent.split("\n").reduce(
  (sections, line) => {
    ...
    if (sectionHeadings.has(trimmed)) {
      const id = trimmed === "About" || trimmed === "Dr. Ankush Garg" ? "about-dr-ankush-garg" : slugify(trimmed);
      const existing = sections.find((section) => section.id === id);
      if (existing) {
        existing.lines.push(line);
        return sections;
      }
      sections.push({ title: trimmed === "About" ? "About Dr. Ankush Garg" : trimmed, id, lines: [] });
      return sections;
    }
...
const articleLinkRules = [
  ...
  { pattern: /\bStress & IBS\b/i, href: "/stress-ibs-support-india" },
  { pattern: /\bIrritable Bowel Syndrome \(IBS\)\b/i, href: "/stress-ibs-support-india" },
  { pattern: /\bIBS\b/i, href: "/stress-ibs-support-india" },
  { pattern: /\bgut-brain axis\b/i, href: "/stress-ibs-support-india" },
...
function ContentSection({ section, sectionIndex, onCitationClick }) {
  ...
  const visualImage = section.id === "the-gut-brain-connection-and-ibs" ? gutBrainImage : section.id === "common-symptoms-of-ibs" || section.id === "why-emotional-wellbeing-matters-in-ibs" ? refluxImage : section.id === "the-neuro-ayurveda-development-system" ? neuroAyurvedaImage : section.id === "managing-ibs-symptoms" ? lifestyleImage : null;
  const isGutBrainSection = section.id === "the-gut-brain-connection-and-ibs";
  const visualImageHeight = isGutBrainSection ? "h-[460px] sm:h-[560px]" : "h-[220px]";
  const visualImageFit = isGutBrainSection ? "object-contain" : "object-cover";
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="bg-[#f3eadb] p-6 sm:p-8">
          ...
          <p className="mt-3 text-[13px] font-semibold leading-6 text-[#111827]">
            Join our mind-body health newsletter for IBS and gut-brain wellbeing guidance.
          </p>
          ...
          <SidebarCard className="border-[#d8c6e5] bg-[#fcf9ff]">
            <a href="/about/doctor" className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-[12px] font-bold leading-5 text-[#75677d]">Clinical Focus: Mind-Body Health, Gut-Brain Wellbeing and Mental Health</p>
